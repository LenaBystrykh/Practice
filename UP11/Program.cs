...
namespace UP11
{
    // Задача 11. №839. зафиксируем натуральное k и перестановку чисел 1, ..., k 
    // (ее можно задать с помощью последовательности натуральных чисел p₁, ..., pₖ, в которую входит каждое из чисел 1, ..., k). 
    // При шифровке в исходном тексте к каждой из последовательных групп по k символов применяется зафиксированная перестановка. 
    // Пусть k = 4 и перестановка есть 3, 2, 4, 1. Тогда группа символов s₁, s₂, s₃, s₄ заменяется на s₃, s₂, s₄, s₁. 
    ...
    public class Program
    {
        ...
        {
            // Ввод (с проверкой) количества элементов в шифрующем наборе
            int k;
            k = CheckInt("Введите k");
            // Создание массива, хранящего шифрующий набор - порядок перестановки символов4
